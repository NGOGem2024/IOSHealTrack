...
const PatientDocumentUploader: React.FC<PatientDocumentUploaderProps> = ({
  ...
  const [localDocument, setLocalDocument] = React.useState<DocumentInfo | null>(initialDocument || null);
  const [showNameModal, setShowNameModal] = React.useState(false);
  const [pendingDocument, setPendingDocument] = React.useState<PendingDocument | null>(null);
  const [documentName, setDocumentName] = React.useState('');
  const [selectedDocumentType, setSelectedDocumentType] = React.useState('Medical Report');
  const [uploadType, setUploadType] = React.useState<'document' | 'photo'>('document');
  ...
  const supportedTypes = [DocumentPicker.types.pdf, DocumentPicker.types.images];
  ...
  const getFileIcon = (fileType: string | undefined, fileName: string) => {
    ...
    } else if (type.includes('image') || ['jpg', 'jpeg', 'png', 'gif'].includes(extension)) {
  ...
  const handleDocumentPick = async (isPhotoOnly = false) => {
    try {
      const pickerTypes = isPhotoOnly ? [DocumentPicker.types.images] : supportedTypes;
      ...
      if (result.size && result.size > maxSize) {
        Alert.alert('File Too Large', 'Please select a file smaller than 10MB', [{ text: 'OK' }]);
        return;
      }
      ...
      const nameWithoutExtension = documentName.split('.').slice(0, -1).join('.');
  ...
  const handleDocumentPress = () => {
    if (localDocument && !isUploading) {
      Alert.alert(
        'Document Options',
        `What would you like to do with "${localDocument.document_name || localDocument.name}"?`,
        [
          { text: 'Remove Document', onPress: handleRemoveDocument, style: 'destructive' },
      ...
      <View style={styles.documentTypeContainer}>
        ...
        <ScrollView
          ...
          {documentTypes.map(type => (
            <TouchableOpacity
              key={type}
              style={[styles.documentTypeChip, selectedDocumentType === type && styles.selectedDocumentTypeChip]}
              onPress={() => setSelectedDocumentType(type)}>
              <Text
                style={[styles.documentTypeChipText, selectedDocumentType === type && styles.selectedDocumentTypeChipText]}>
                {type}
              </Text>
            </TouchableOpacity>
          ))}
      ...
      <View style={styles.uploadButtonsContainer}>
        ...
        <TouchableOpacity
          ...
          <View style={styles.uploadButtonContent}>
            ...
            <Text style={[styles.uploadButtonText, styles.photoButtonText]}>Add Photo</Text>
          ...
          <View style={styles.modalContainer}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>
                {uploadType === 'photo' ? 'Photo Information' : 'Document Information'}
              </Text>
            ...
            <ScrollView style={styles.modalContent} showsVerticalScrollIndicator={false}>
              {pendingDocument && (
                <View style={styles.filePreview}>
                  <View style={styles.filePreviewIcon}>
                    <Icon
                      name={getFileIcon(pendingDocument.type, pendingDocument.name)}
                      size={30}
                      color={getFileIconColor(pendingDocument.type, pendingDocument.name)}
                    />
                  </View>
              ...
              <View style={styles.inputContainer}>
                ...
                <TextInput
                  ...
                  placeholder={uploadType === 'photo' ? 'Enter photo name' : 'Enter document name'}
            ...
            <View style={styles.modalFooter}>
              ...
              <TouchableOpacity
                style={[styles.confirmButton, !documentName.trim() && styles.disabledButton]}
...
const getStyles = (theme: ReturnType<typeof getTheme>) => StyleSheet.create({
  ...
  uploadButton: {
    ...
    backgroundColor: theme.colors.card === '#FFFFFF' ? 'rgba(245, 250, 255, 0.5)' : 'rgba(17, 159, 179, 0.05)',
    minHeight: 100,
  },
  documentButton: {
    borderColor: '#007B8E',
  ...
  documentTypeChip: {
    backgroundColor: theme.colors.card === '#FFFFFF' ? '#f0f0f0' : 'rgba(17, 159, 179, 0.1)',
